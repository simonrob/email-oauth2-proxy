...
two overridable methods `receive_from_client` and `receive_from_server` give access to raw IMAP/SMTP messages/commands
as they are received. The return values from the overridden methods and the two sender methods `send_to_server` and
`send_to_client` allow you to send messages/commands to the client/server (and other chained plugins) in response. The
`log_debug`, `log_info` and `log_error` methods allow you to post messages to the proxy's main log. Note that plugins
are inserted after authentication has finished – you will not receive (or be able to send) anything between the client
or server until that process is complete. """
