...
domains (for example, your own internal domain(s)). If any of these domains are present in the recipient list, the
ability to react to this message will be retained. Note that this functionality applies to all recipients - i.e., if a
single whitelisted recipient is present, all recipients will be able to react to the message. For further details about
disabling reactions, see the Microsoft article: https://techcommunity.microsoft.com/t5/outlook-blog/r/ba-p/3928103.
"""
...
class SMTPDisableO365Reactions(plugins.BasePlugin.BasePlugin):
    ...
    def __init__(self, domain_whitelist=None):
        ...
        self.sending_state, self.previous_line_ended, self.whitelist_domain, self.whitelist_domain_logged = self.reset()

    def reset(self):
        self.sending_state = self.STATE.NONE
        self.previous_line_ended = False
        self.whitelist_domain = None
        self.whitelist_domain_logged = False
        return self.sending_state, self.previous_line_ended, self.whitelist_domain, self.whitelist_domain_logged
    ...
    def receive_from_client(self, byte_data):
        ...
        if len(byte_data) == 6 and byte_data.lower() == b'rset\r\n':  # RSET can be sent at any point; discard state
            ...
            if self.whitelist_domain:
                if not self.whitelist_domain_logged:
                    self.whitelist_domain_logged = True
                ...
                byte_data_lines = byte_data.split(b'\r\n')
                for i, item in enumerate(byte_data_lines):
                    if item.startswith(b'Content-Type: '):
                        byte_data_lines[i] = b'x-ms-reactions: disallow\r\n' + item
                        self.log_debug('Added `x-ms-reactions: disallow` header to outgoing message')
                        break
                byte_data = b'\r\n'.join(byte_data_lines)
