...
- `--aws-secrets` enables the proxy to store OAuth 2.0 tokens remotely in [AWS Secrets Manager](https://aws.amazon.com/secrets-manager/), rather than storing them in the local configuration file. This will only be applied to accounts which have an `aws_secret` parameter configured in the local configuration file, containing the ARN or name of the secret. To use this feature, you must install the requirements in `requirements-aws-secrets.txt` and [set up authentication credentials for your AWS account](https://boto3.amazonaws.com/v1/documentation/api/latest/guide/quickstart.html#configuration).
...
The proxy can be packaged as a single self-contained executable using [pyinstaller](https://pyinstaller.org/) if desired: `pyinstaller --onefile emailproxy.py`.
